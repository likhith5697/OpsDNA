...
export const IncidentDetail: React.FC<IncidentDetailProps> = ({ incident, onLogOpen, onToast }) => {
  ...
  useEffect(() => {
    getPods()
      ...
      .catch(() => {
        setPods([
          { name: `${incident.cmdb_ci}-7d4f9b-xk2zp`, status: 'CrashLoopBackOff', restarts: 14, age: '3h', ready: '0/1' },
          { name: `${incident.cmdb_ci}-7d4f9b-mn8pq`, status: 'Running', restarts: 2, age: '3h', ready: '1/1' },
          { name: `${incident.cmdb_ci}-7d4f9b-rt5wv`, status: 'OOMKilled', restarts: 8, age: '3h', ready: '0/1' },
        ]);
      });
  }, [incident.cmdb_ci]);
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto pb-20">
        ...
        <div className="bg-[#161B22] border border-[#30363D] rounded-xl p-4 mb-4">
          ...
          <div className="space-y-2 text-sm text-gray-300">
            <div>
              ...
              <p className="mt-0.5">Memory leak introduced in v2.3.1 of {incident.cmdb_ci}. OOMKill events cascading across pods.</p>
            </div>
            <div>
              <span className="text-gray-500 text-xs">Diagnosis</span>
              <p className="mt-0.5">Deployment rollback to v2.3.0 will restore service. Confirmed stable in staging for 48h.</p>
            </div>
            <div>
      ...
      {isPending && incident.fix_command && (
        <div className="fixed bottom-0 left-[200px] right-[380px] bg-[#161B22] border-t border-amber-500/40 px-6 py-3 flex items-center justify-between z-20">
          <div>
